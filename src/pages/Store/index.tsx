    ...
    if (repository && !data) {
      downloadFile(repository.URL).then(function (response: any) {
        loadBinary(response.download.file).then(function (response: any) {
          ...
          sql(
            `UPDATE repositories SET name = ${escape(data.name)}, icon ${escape(data.icon)} WHERE id = ${escape(params.id)})`
          ).then(() => {
            setData(data);
          });
        });
                  ...
                  <div key={app.name} className="bg-core-black-contrast-2 rounded flex justify-start items-left h-full">
                    ...
                    <div className="flex-grow flex items-center px-4 truncate overflow-hidden min-w-[0]">
                      <div className="overflow-hidden truncate">
                        <div className="text-ellipsis overflow-hidden whitespace-nowrap">{app.name}</div>
